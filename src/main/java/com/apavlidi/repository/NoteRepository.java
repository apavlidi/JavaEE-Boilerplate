...
@ApplicationScoped
@Repository
public abstract class NoteRepository implements EntityRepository<Note, Long>,
    EntityManagerDelegate<Note> {
  ...
  public Optional<Note> findNoteByText(String text) {
    try {
      return em.createQuery("select DISTINCT(t) from Note t where t.text=:text",
          Note.class).setParameter("text", text).getResultList().stream().findFirst();
    } catch (
        NoResultException nre) {
      return Optional.empty();
    }
